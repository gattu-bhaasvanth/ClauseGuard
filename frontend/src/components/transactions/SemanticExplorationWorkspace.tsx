...
export function SemanticExplorationWorkspace({
  ...
  const handleSearch = async (questionToAsk?: string) => {
    ...
    if (inputRef.current) {
      inputRef.current.value = textToQuery;
    }
    setIsLoading(true);
    setError(null);
  ...
  const handleSelectSuggestedQuery = (suggestedText: string) => {
    // 1. Immediately set the React state
    setQuery(suggestedText);
    // 2. Immediately set the DOM input element value so element value === selected question
    if (inputRef.current) {
      inputRef.current.value = suggestedText;
    }
    // 3. Trigger query submission
